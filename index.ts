 ...
 * ```
 */
export function batch<T = TrackingInfo>(send: SendFunction<T>, coalesce: CoalesceFunction<T> = identity): TrackingSubscriber {
    ...
    return function collect(entry: TrackingInfo): void {
        queue.push(entry);
        if (scheduled)
            return;
        scheduled = true;
